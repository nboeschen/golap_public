...
def plot(files,title,x_var,metric="time_ms",sort=True,shape="bar",with_optimal_ssd_scan=False,with_optimal_inmem_scan=False):
    ...
    axes.set_xticklabels(list(x if "+Pruning" not in x else "+Pruning" for x in order.keys()),fontsize=26)
...
if len(sys.argv) > 1 and sys.argv[1] == "print":
    plt.savefig(f"pdf/front_extended.pdf",metadata=common.EMPTY_PDF_META)
else:
    plt.show()
