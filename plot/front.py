...
def plot(files,title,x_var,metric="time_ms",sort=True,shape="bar",with_optimal_ssd_scan=False,with_optimal_inmem_scan=False):
    ...
    order={" InMemory":0," Uncompressed\nSSD":1," Uncompressed\nSSD\n+Pruning":2,
            " Compressed\nSSD":2," Compressed\nSSD\n+Pruning":3,
            # "Golap (GPU, ours) UNCOMPRESSED",
            "Golap\nCompressed SSD ":4.5, "+ Pruning ":5.5}
    ...
    for label,data_or_path in files:
        ...
        for comp_algo,cur_data in data.groupby("comp_algo"):
            axes.bar(order[f"{label} {comp_algo}"],cur_data[metric],width=1,edgecolor="black",
                     linewidth=2, **common.CONFIG[f"{label} {comp_algo}"])

    common.axes_prepare(axes,metric)

    axes.set_xticks(list(order.values()))
    axes.set_xticklabels(order.keys(),fontsize=25)
...
SSB_HANDCODED_OPT = pd.read_csv("./handcoded_optimized_export_1.csv",comment="#")
...
for index, row in SSB_HANDCODED_OPT.iterrows():
    if row.strategy == "SSD":
        if row.prune:
            if row.compress:
                SSB_HANDCODED_OPT.at[index,"comp_algo"] = "Compressed\n"+row.strategy+"\n+Pruning"
            else:
                # uncompressed pruning not so interesting
                SSB_HANDCODED_OPT.drop([index], axis='index',inplace=True)
                # SSB_HANDCODED_OPT.at[index,"comp_algo"] = "Uncompressed\n"+row.strategy+"\n+Pruning"
        else:
            if row.compress:
                SSB_HANDCODED_OPT.at[index,"comp_algo"] = "Compressed\n"+row.strategy
            else:
        ...
        SSB_HANDCODED_OPT.at[index,"comp_algo"] = "InMemory"


files = [
             # ["DuckDB", SSB_DUCKDB],
             # ["Golap (GPU, ours)", SSB_GOLAP_UNCOMPRESSED],
             ["Golap\nCompressed SSD", SSB_GOLAP_DATA],
...
axes.set_xlabel("    └─────────────────  CPU  ────────────────┘     └────────  GPU  ───────┘",loc="left",weight="bold")


axes.text(0.8,35,"UNCOMPRESSED\n(I/O bound)",fontsize=43,rotation=90,c="#444444")
axes.text(2.25,50,"COMPRESSED\n(CPU bound)",fontsize=43,rotation=90,c="#444444")
