...
DEFINE_string(dbpath, "/raid/gds/tpch/parquet10/uncompressed/", "Path to directory containing the parquet files, load.sql and schema.sql.");
DEFINE_string(query, "query1", "The TPCH queries to execute.");
...
int main(int argc, char *argv[]){
    ...
    for (uint32_t i = 0;i<FLAGS_repeat; ++i){
        for (auto &query: queries){
            ...
            std::unique_ptr<duckdb::MaterializedQueryResult> parquet_meta = con.Query("SELECT path_in_schema,compression,SUM(total_uncompressed_size),SUM(total_compressed_size) FROM parquet_metadata('"+FLAGS_dbpath+"lineitem.parquet') GROUP BY path_in_schema,compression;");
            ...
            if (prepared->HasError()) {
                std::cout << "Prepare Error: " << prepared->GetError() << "\n";
                continue;
            }
            ...
            if (result->HasError()) {
                std::cout << "Execute Error: " << prepared->GetError() << "\n";
                continue;
            }
