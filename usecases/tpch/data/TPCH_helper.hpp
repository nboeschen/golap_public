...
static const std::vector<std::string> SHIPINSTR = {"DELIVER IN PERSON","COLLECT COD","TAKE BACK RETURN"};
static const std::vector<std::string> SHIPMODE = {"REG AIR","AIR","RAIL","SHIP","TRUCK","MAIL","FOB"};
static const std::vector<char> RFLAG = {'R','A'};
...
static int64_t partkey_to_suppkey(uint64_t p, int snum, uint64_t supplier_num){
    return (p + snum * (supplier_num / 4 + (long) ((p - 1) / supplier_num))) % supplier_num + 1;
}
