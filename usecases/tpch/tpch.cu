...
int main(int argc, char *argv[]){
    ...
    TPCHVar var{FLAGS_cuda_device, FLAGS_block_limit, "", "SSD2GPU", FLAGS_scale_factor, FLAGS_core_pin, 8, 0,
    ...
    TPCHColLayout db(var, FLAGS_dbpath == "" ? "init_populate" : "init_only");
    if (FLAGS_dbpath != ""){
        ...
        else if (util::ends_with(FLAGS_dbpath,".csv")) golap::read_col_db_csv(db.tables,FLAGS_dbpath,FLAGS_csv_delimiter,0);
        else {
    ...
    std::cout << "# Loaded data from disk." << std::endl;
        ...
        for (auto &sort_by: sort_bys){
            ...
            for (auto &dataflow: dataflows){
                ...
                for (auto &query_func_name: query_funcs){
                    auto query_func = &TPCHColLayout::query1inmem;
