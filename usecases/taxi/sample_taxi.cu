...
DEFINE_uint32(scale_factor, 10, "db scale factor.");
...
int main(int argc, char *argv[]){
    ...
    TaxiVar var{FLAGS_cuda_device, FLAGS_block_limit, "", "", FLAGS_scale_factor, "", FLAGS_workers, 0,
    ...
    TaxiColLayout db(var, FLAGS_init_variant);
    if (FLAGS_dbpath != ""){
    ...
    for (int i = 0;i<FLAGS_repeat; ++i){
        for (auto &sample_ratio: sample_ratios){
            ...
            db.tables.trips.apply([&](auto& a_col, uint64_t num_tuples, uint64_t col_idx){
