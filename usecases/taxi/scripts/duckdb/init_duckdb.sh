...
csv_folder="/mnt/labstore/nboeschen/taxi/"
parquet_folder="/raid/gds/taxi/parquet${scale_factor}"
...
# also add an inmem variant, which loads the full uncompressed parquet on import:
mkdir -p "${parquet_folder}/inmem"
sed '/enable_progress_bar/Q' csv_to_parquet.sql > "${parquet_folder}/inmem/schema.sql"
echo "COPY trips FROM '${parquet_folder}/inmem/trips.parquet' (FORMAT 'parquet', CODEC 'UNCOMPRESSED');" > "${parquet_folder}/inmem/load.sql"
ln -s "${parquet_folder}/uncompressed/trips.parquet" "${parquet_folder}/inmem/"
