...
DEFINE_string(dbpath, "/raid/gds/ssb/parquet10/uncomp/", "Path to directory containing the parquet files, load.sql and schema.sql.");
DEFINE_string(query, "query1.1", "The SSB queries to execute.");
...
DEFINE_bool(perfevent, false, "Gather and print perfevent info.");
DEFINE_bool(explain_analyze, false, "Run query under the 'explain analyze' prefix.");
...
struct DuckDBVar{
    ...
    std::string to_csv(){
        std::stringstream ss;
        ss << query << "," <<scale_factor<<"," <<drop_cache<<","<<row_group_size<<","<<comp_algo << "," <<mem_limit << ","
        << thread_limit <<","<< uncomp_bytes << "," << comp_bytes <<","<<io_bytes<< "," << time_ms;
        return ss.str();
    }
    ...
    static std::string csv_header(){
        return "query,scale_factor,drop_cache,row_group_size,comp_algo,mem_limit,thread_limit,uncomp_bytes,comp_bytes,io_bytes,time_ms";
    }
...
int main(int argc, char *argv[]){
    ...
    // if duckdb src is used:
    // con.Query(std::string("INSTALL parquet;"));
    // con.Query(std::string("LOAD parquet;"));
    ...
    for (uint32_t i = 0;i<FLAGS_repeat; ++i){
        for (auto &query: queries){
            ...
            auto filepath = FLAGS_dbpath+"/lineorder.parquet";
            if (FLAGS_dbpath.find("sorted") != std::string::npos) filepath += "/data_0.parquet";

            std::unique_ptr<duckdb::MaterializedQueryResult> parquet_meta = con.Query("SELECT path_in_schema,compression,SUM(total_uncompressed_size),SUM(total_compressed_size),MAX(row_group_num_rows) FROM parquet_metadata('"+filepath+"') GROUP BY path_in_schema,compression;");
            var.comp_algo = parquet_meta->GetValue(1,0).ToString();
            var.row_group_size = parquet_meta->GetValue(4,0).GetValue<uint32_t>();

            auto query_txt = get_query_text(query);
            if (FLAGS_explain_analyze) query_txt = std::string("EXPLAIN ANALYZE ") + query_txt;
            ...
            if (FLAGS_perfevent) e.startCounters();
            ...
            if (FLAGS_print_res){
                materialized->Print();
            }else if (FLAGS_explain_analyze) {
                util::Log::get().warn_fmt("Run the binary again with --print_res to get EXPLAIN ANALYZE results");
            }
