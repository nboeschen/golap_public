...
for idx,query_name in enumerate(["Query3.1 - Least Selective","Query3.2 - More Selective",
    ...
    for title,data in files:
        ...
        bars.add_cat(cur_data["customer_factor"],cur_data["time_ms"]/1000,label=title,part_of=title)
...
if len(sys.argv) > 1 and sys.argv[1] == "print":
    plt.savefig(f"pdf/hybrid_exec_time.pdf",metadata=common.EMPTY_PDF_META)
else:
    plt.show()
