...
DEFINE_uint32(extra_workers, 1, "Number of extra independent execution workers (e.g. query3.1a customer).");
DEFINE_uint32(add_cpu_threads, 1, "Number of additional cpu threads.");
...
DEFINE_string(query, "", "SSB querys to execute, delimited by comma.");
...
DEFINE_bool(join_prune, false, "Runtime join prune");
DEFINE_bool(extended_price_pred, false, "Extended price predicate");
DEFINE_bool(event_sync, true, "Sync inside a pipeline. true: sync event (allow overlap), false: sync stream (no overlap)");
DEFINE_string(aws_endpoint, "default", "AWS Profile Name");

DEFINE_bool(verify,false,"Verify Result.");
DEFINE_string(sort_by, "natural", "Column to sort on"); // order of parameters here matters
...
DEFINE_double(bloomepsilon, 0.1, "False positive rate of bloomfilters. {ONLY FOR Q2a queries}");
...
int main(int argc, char *argv[]){
    ...
    SSBVar var{FLAGS_cuda_device, FLAGS_block_limit, "", "SSD2GPU", FLAGS_scale_factor, FLAGS_core_pin, 8, 0,
    ...
    var.extra_workers = FLAGS_extra_workers;
    var.add_cpu_threads = FLAGS_add_cpu_threads;
    var.bloomepsilon = FLAGS_bloomepsilon;
    var.join_prune = FLAGS_join_prune;
    var.extended_price_pred = FLAGS_extended_price_pred;
    ...
    SSBColLayout ssb(var, FLAGS_dbpath == "" ? "init_populate" : "init_only");
    if (FLAGS_dbpath != ""){
        util::Log::get().info_fmt("Loading table data from disk ...");
        if (util::ends_with(FLAGS_dbpath,".dat")) golap::read_col_db_bin(ssb.tables,FLAGS_dbpath);
        else if (util::ends_with(FLAGS_dbpath,".csv")) golap::read_col_db_csv(ssb.tables,FLAGS_dbpath,FLAGS_csv_delimiter);
        else {
    ...
    if (FLAGS_query.find("query") != std::string::npos){
        ...
        SortHelper::get().prejoin_tables(ssb.tables, var.chunk_size_vec);
    }

    if (FLAGS_query.find("pre_aggr") != std::string::npos){
        if (FLAGS_pre_aggr_path == ""){
            util::Log::get().info_fmt("Preaggr tables ...");
            SortHelper::get().preaggr_tables(ssb.tables, var.chunk_size_vec);
        } else{
            util::Log::get().info_fmt("Loading preaggr data from disk ...");
            SortHelper::get().init_pre_aggr(ssb.tables.lineorder.num_tuples);
            if (util::ends_with(FLAGS_pre_aggr_path,".dat")) golap::read_col_db_bin(*SortHelper::get().preaggrdb,FLAGS_pre_aggr_path);
            else if (util::ends_with(FLAGS_pre_aggr_path,".csv")) golap::read_col_db_csv(*SortHelper::get().preaggrdb,FLAGS_pre_aggr_path,FLAGS_csv_delimiter,0);
            else {
                std::cout << "Unknown file extension for pre_aggr diskdb, exiting!\n";
                return 1;
            }
        }
        ...
        for (auto &customer_factor: customer_factors){
            ...
            for (auto &sort_by: sort_bys){
                if (var.sort_by == sort_by) { /*pass*/
                    ...
                    helper.apply(ssb.tables.lineorder, var.chunk_size_vec);
                    // todo: the prejoined attributes should be sorted equally
                }else{
                    auto& helper = SortHelper::get();
                    util::Log::get().info_fmt("Sorting fact table ...");
                    helper.apply(sort_by, ssb.tables, var.chunk_size_vec);
                }
                var.sort_by = sort_by;

                // ssb.tables.lineorder.to_csv(std::cout," | ",0,50);

                if (FLAGS_shuffle_ratio > 0.0){
                    ssb.tables.lineorder.shuffle_ratio(FLAGS_shuffle_ratio);
                }
                for (auto &dataflow: dataflows){
                    var.dataflow = dataflow;
                    for (auto &query_func_name: query_funcs){
                        auto query_func = &SSBColLayout::select_revenue;
