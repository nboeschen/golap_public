...
bool query3a(SSBColLayout &ssbobj, KERNEL kernel, CUSTOMER_KERNEL customer_kernel, Q3SUPP_COLUMNS &supplier_dev,
    ...
    for (uint32_t pipeline_idx=0; pipeline_idx<ssbobj.var.extra_workers; ++pipeline_idx){
        extra_p_threads.emplace_back([&,pipeline_idx]{
            ...
            while(env.rootop.step(env.rootstream.stream,env.rootevent.event)){
                ...
                pipeline_customer_q3a<<<num_blocks,512,0,env.rootstream.stream>>>(
                                                                            // customer_hj,
                ...
                for(uint64_t r_id=cust_host_insert; r_id<cust_host_insert+cust_tuples; ++r_id){

                    customer_hj.insert(r_id, customer_host.ptr<Q3Customer>()[r_id].c_key);
                }
...
bool query3_1a_gen(SSBColLayout &ssbobj, KERNEL kernel, CUSTOMER_KERNEL customer_kernel){
    ...
    if(ssbobj.var.comp_algo == "UNCOMPRESSED"){
        return query3a<KERNEL,CUSTOMER_KERNEL,LO_GROUP,GROUP,golap::LoadEnv,
    ...
    }else return query3a<KERNEL,CUSTOMER_KERNEL,LO_GROUP,GROUP,golap::DecompressEnv,
                    golap::PredInfo<decltype(Supplier::s_region),RegionAsiaPred>,
                    golap::PredInfo<decltype(Date::d_year),DATE_9297>>(ssbobj, kernel, customer_kernel, *supplier_dev, *date_dev, supp_pred, date_pred);
}

template <typename KERNEL, typename CUSTOMER_KERNEL, typename LO_GROUP, typename GROUP>
bool query3_2a_gen(SSBColLayout &ssbobj, KERNEL kernel, CUSTOMER_KERNEL customer_kernel){
    auto date_dev = prepareQ3DATE(ssbobj);
    auto supplier_dev = prepareQ3SUPP(ssbobj);
...
template <typename KERNEL, typename CUSTOMER_KERNEL, typename LO_GROUP, typename GROUP>
bool query3_3a_gen(SSBColLayout &ssbobj, KERNEL kernel, CUSTOMER_KERNEL customer_kernel){
    auto date_dev = prepareQ3DATE(ssbobj);
    auto supplier_dev = prepareQ3SUPP(ssbobj);
...
template <typename KERNEL, typename CUSTOMER_KERNEL, typename LO_GROUP, typename GROUP>
bool query3_4a_gen(SSBColLayout &ssbobj, KERNEL kernel, CUSTOMER_KERNEL customer_kernel){
    auto date_dev = prepareQ3DATE(ssbobj);
    auto supplier_dev = prepareQ3SUPP(ssbobj);
...
bool SSBColLayout::query3_1a(){
    return query3_1a_gen<decltype(pipeline_q3a<Q3PreJoinedNation>),decltype(pipeline_customer_q3c<decltype(Customer::c_region),RegionAsiaPred,golap::PredInfo>),Q3PreJoinedNation,Q3NATION_GROUP>(*this,pipeline_q3a<Q3PreJoinedNation>,pipeline_customer_q3c<decltype(Customer::c_region),RegionAsiaPred,golap::PredInfo>);
}

bool SSBColLayout::query3_2a(){
    return query3_2a_gen<decltype(pipeline_q3a<Q3PreJoinedCity>),decltype(pipeline_customer_q3c<decltype(Customer::c_nation),NationUSPred,golap::PredInfo>),Q3PreJoinedCity,Q3CITY_GROUP>(*this,pipeline_q3a<Q3PreJoinedCity>,pipeline_customer_q3c<decltype(Customer::c_nation),NationUSPred,golap::PredInfo>);
}

bool SSBColLayout::query3_3a(){
    return query3_3a_gen<decltype(pipeline_q3a<Q3PreJoinedCity>),decltype(pipeline_customer_q3c<decltype(Customer::c_city),CityKIPred,golap::PredInfo>),Q3PreJoinedCity,Q3CITY_GROUP>(*this,pipeline_q3a<Q3PreJoinedCity>,pipeline_customer_q3c<decltype(Customer::c_city),CityKIPred,golap::PredInfo>);
}

bool SSBColLayout::query3_4a(){
    return query3_4a_gen<decltype(pipeline_q3a<Q3PreJoinedCity>),decltype(pipeline_customer_q3c<decltype(Customer::c_city),CityKIPred,golap::PredInfo>),Q3PreJoinedCity,Q3CITY_GROUP>(*this,pipeline_q3a<Q3PreJoinedCity>,pipeline_customer_q3c<decltype(Customer::c_city),CityKIPred,golap::PredInfo>);
}
