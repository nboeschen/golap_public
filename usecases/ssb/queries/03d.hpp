...
template <typename KERNEL, typename LO_GROUP, typename GROUP, typename BLOCK_ENV, typename CUST_BLOCK_ENV, typename SUPP_PRED, typename DATE_PRED>
bool query3d(SSBColLayout &ssbobj, KERNEL kernel, Q3SUPP_COLUMNS &supplier_dev,
    ...
    */
    ...
    ssbobj.tables.lineorder.apply([&](auto& a_col, uint64_t num_tuples, uint64_t col_idx){
    ...
    ssbobj.tables.customer.apply([&](auto& a_col, uint64_t num_tuples, uint64_t col_idx){
        auto entry = columns.find(a_col.attr_name);
        if (entry == columns.end()) return;
        auto algo = ssbobj.var.comp_algo;
        if(ssbobj.var.comp_algo == "BEST_BW_COMP"){
    ...
    // Host Buffer of qualified customer tuples
    ...
    golap::HostHashMap customer_hj(ssbobj.tables.customer.num_tuples, customer_host.ptr<Q3Customer>());
    ...
    std::vector<golap::TableLoader<CUST_BLOCK_ENV>> customer_envs;
    ...
    for (uint32_t pipeline_idx=0; pipeline_idx<ssbobj.var.extra_workers; ++pipeline_idx){
        ...
        // customer_envs.emplace_back(startblock, endblock, compinfos["c_key"], compinfos["c_city"],
        //                            compinfos["c_nation"], compinfos["c_region"], cust_tuples_per_chunk);
        customer_envs.emplace_back(4);
        customer_envs[pipeline_idx].add("c_key", all_blocks_idxs, startblock, endblock, compinfos["c_key"], nvcomp::TypeOf<CKEY_TYPE>(), "SSD2CPU");
        customer_envs[pipeline_idx].add("c_city", all_blocks_idxs, startblock, endblock, compinfos["c_city"], nvcomp::TypeOf<uint8_t>(), "SSD2CPU");
        customer_envs[pipeline_idx].add("c_nation", all_blocks_idxs, startblock, endblock, compinfos["c_nation"], nvcomp::TypeOf<uint8_t>(), "SSD2CPU");
        customer_envs[pipeline_idx].add("c_region", all_blocks_idxs, startblock, endblock, compinfos["c_region"], nvcomp::TypeOf<uint8_t>(), "SSD2CPU");
    }
    ...
    for (uint32_t pipeline_idx=0; pipeline_idx<ssbobj.var.extra_workers; ++pipeline_idx){
        extra_p_threads.emplace_back([&,pipeline_idx]{
            ...
            uint64_t round = 0;
            uint64_t tuples_this_round,cust_qualified,cust_host_insert;
            while(env.rootop.step(env.rootstream.stream,env.rootevent.event)){
                ...
                /*
                Go through all tuples and add them in the cpu hashtable
                TODO: add in batches instead of single tuples
                 */
                cust_qualified = 0;
                for(uint64_t r_id=0; r_id<tuples_this_round; ++r_id){
                    if (!cust_pred.pred(&cust_pred.col[r_id])) continue;
                    cust_qualified += 1;

                    cust_host_insert = customer_host_counter.fetch_add(1,std::memory_order_relaxed);

                    customer_host.ptr<Q3Customer>()[cust_host_insert] = Q3Customer{
                                                                env.blockenvs.at("c_key").decomp_buf.template ptr<CKEY_TYPE>()[r_id],
                                                                env.blockenvs.at("c_city").decomp_buf.template ptr<CCITY_TYPE>()[r_id],
                                                                env.blockenvs.at("c_nation").decomp_buf.template ptr<CNATION_TYPE>()[r_id],
                                                                env.blockenvs.at("c_region").decomp_buf.template ptr<CREGION_TYPE>()[r_id]};


                    customer_hj.insert(cust_host_insert, env.blockenvs.at("c_key").decomp_buf.template ptr<CKEY_TYPE>()[r_id]);
                }
                // util::Log::get().info_fmt("Pipeline/Thread[%lu]: Qualified %lu / %lu customer tuples (%.2f %%)!",pipeline_idx,
                //                           cust_qualified,tuples_this_round,100.0*cust_qualified/tuples_this_round);
    ...
    // util::Log::get().info_fmt("Customer HT on host took %2.fms",customer_timer.elapsed());
    ...
    util::Log::get().info_fmt("CUST vs LO timer          =CUST %.2f, LO %.2f",customer_time,lo_time);
    util::Log::get().info_fmt("Customer lines in HT %lu (%lu bytes)",customer_host_counter.load(),customer_host_counter*sizeof(Q3Customer));
...
template <typename KERNEL, typename LO_GROUP, typename GROUP>
bool query3_1d_gen(SSBColLayout &ssbobj, KERNEL kernel){
    ...
    // CPU only uses UNCOMPRESSED, since its faster
    if(ssbobj.var.comp_algo == "UNCOMPRESSED"){
        return query3d<KERNEL,LO_GROUP,GROUP,golap::LoadEnv, golap::LoadChunkEnvCPU,
                            golap::PredInfo<decltype(Supplier::s_region),RegionAsiaPred>,
                            golap::PredInfo<decltype(Date::d_year),DATE_9297>>(ssbobj, kernel, *supplier_dev, *date_dev, supp_pred, date_pred);
    }else return query3d<KERNEL,LO_GROUP,GROUP,golap::DecompressEnv, golap::LoadChunkEnvCPU,
                    golap::PredInfo<decltype(Supplier::s_region),RegionAsiaPred>,
                    golap::PredInfo<decltype(Date::d_year),DATE_9297>>(ssbobj, kernel, *supplier_dev, *date_dev, supp_pred, date_pred);
}

template <typename KERNEL, typename LO_GROUP, typename GROUP>
bool query3_2d_gen(SSBColLayout &ssbobj, KERNEL kernel){
    auto date_dev = prepareQ3DATE(ssbobj);
    auto supplier_dev = prepareQ3SUPP(ssbobj);
...
template <typename KERNEL, typename LO_GROUP, typename GROUP>
bool query3_3d_gen(SSBColLayout &ssbobj, KERNEL kernel){
    auto date_dev = prepareQ3DATE(ssbobj);
    auto supplier_dev = prepareQ3SUPP(ssbobj);
...
template <typename KERNEL, typename LO_GROUP, typename GROUP>
bool query3_4d_gen(SSBColLayout &ssbobj, KERNEL kernel){
    auto date_dev = prepareQ3DATE(ssbobj);
    auto supplier_dev = prepareQ3SUPP(ssbobj);
    ...
    if(ssbobj.var.comp_algo == "UNCOMPRESSED"){
        return query3d<KERNEL,LO_GROUP,GROUP,golap::LoadEnv, golap::LoadChunkEnvCPU,
                            golap::PredInfo<decltype(Supplier::s_city),CityKIPred>,
                            golap::PredInfo<decltype(Date::d_yearmonth),YEARMONTHDec1997>>(ssbobj, kernel, *supplier_dev, *date_dev, supp_pred, date_pred);
    }else return query3d<KERNEL,LO_GROUP,GROUP,golap::DecompressEnv, golap::LoadChunkEnvCPU,
                    golap::PredInfo<decltype(Supplier::s_city),CityKIPred>,
                    golap::PredInfo<decltype(Date::d_yearmonth),YEARMONTHDec1997>>(ssbobj, kernel, *supplier_dev, *date_dev, supp_pred, date_pred);
}
...
bool SSBColLayout::query3_1d(){
    return query3_1d_gen<decltype(pipeline_q3a<Q3PreJoinedNation>),Q3PreJoinedNation,Q3NATION_GROUP>(*this,pipeline_q3a<Q3PreJoinedNation>);
}

bool SSBColLayout::query3_2d(){
    return query3_2d_gen<decltype(pipeline_q3a<Q3PreJoinedCity>),Q3PreJoinedCity,Q3CITY_GROUP>(*this,pipeline_q3a<Q3PreJoinedCity>);
}

bool SSBColLayout::query3_3d(){
    return query3_3d_gen<decltype(pipeline_q3a<Q3PreJoinedCity>),Q3PreJoinedCity,Q3CITY_GROUP>(*this,pipeline_q3a<Q3PreJoinedCity>);
}

bool SSBColLayout::query3_4d(){
    return query3_4d_gen<decltype(pipeline_q3a<Q3PreJoinedCity>),Q3PreJoinedCity,Q3CITY_GROUP>(*this,pipeline_q3a<Q3PreJoinedCity>);
}
